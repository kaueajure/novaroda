...
export function CabecalhoPainel({
  ...
  return (
    <header
      className={cn(
        "sticky top-0 z-20 border-b border-linha bg-fundo transition-[margin] duration-200",
      ...
      <div className="flex min-h-16 items-center gap-3 px-4 sm:px-6 lg:px-8">
        ...
        <div className="hidden min-w-0 flex-1 md:block">
          ...
          <p className="text-xs text-texto-fraco">
            {loja.cidade} · operação comercial
          </p>
        </div>

        <label className="foco-visivel flex h-11 min-w-0 flex-1 items-center gap-2 rounded-md border border-linha bg-card-solido px-3 md:max-w-md">
          <Search className="size-4 shrink-0 text-texto-fraco" aria-hidden="true" />
          <span className="sr-only">Buscar no painel</span>
          <input
            type="search"
            placeholder="Buscar placa, modelo, cliente ou proposta"
