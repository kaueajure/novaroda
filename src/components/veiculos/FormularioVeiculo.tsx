...
import type { DadosVeiculo, Veiculo } from "@/types/Veiculo";
    ...
    <form onSubmit={handleSubmit(enviar)} className="grid gap-5 xl:grid-cols-[1fr_360px]" noValidate>
      <section className="rounded-xl border border-linha bg-card p-5">
        <div className="grid gap-4 md:grid-cols-2">
          ...
          />
          <CampoTexto label="Placa opcional" erro={errors.placa?.message} {...register("placa")} />
        ...
        <label className="mt-4 flex min-h-11 items-center gap-3 rounded-lg border border-linha bg-white/[0.025] px-3 text-sm font-semibold text-texto-suave">
          <input
            type="checkbox"
            className="size-4 rounded border-linha accent-[#61d6c8]"
            {...register("destaque")}
          />
          Destacar veículo na loja
        </label>

        <div className="mt-6 flex flex-col-reverse gap-3 sm:flex-row sm:justify-end">
          <Link
            href="/painel/veiculos"
            className="foco-visivel inline-flex min-h-11 items-center justify-center gap-2 rounded-lg border border-linha bg-white/[0.03] px-4 py-2 text-sm font-semibold text-texto-suave transition duration-200 hover:border-linha-forte hover:bg-white/[0.07] hover:text-texto"
          >
          ...
          >
            {modo === "cadastro" ? "Cadastrar veículo" : "Salvar alterações"}
          </BotaoPrimario>
        </div>
      </section>
      ...
      <aside className="rounded-xl border border-linha bg-card p-5">
        ...
        <p className="mt-4 text-sm leading-6 text-texto-suave">
          Uma ficha completa reduz perguntas repetidas no atendimento e deixa o
          estoque mais fácil de vender.
        </p>
      </aside>
    </form>
  );
