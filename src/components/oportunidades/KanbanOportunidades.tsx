    ...
    <div className="overflow-x-auto pb-2">
      <div className="grid min-w-[1120px] grid-cols-6 gap-4">
        {etapas.map((etapa) => {
          ...
          return (
            <section key={etapa} className="rounded-xl border border-linha bg-card p-3">
              <div className="mb-3 flex items-center justify-between gap-3 px-1">
                <h2 className="font-display text-xl font-semibold text-texto">
                  {rotuloEtapaOportunidade[etapa]}
                </h2>
                <span className="rounded-full border border-linha bg-white/[0.035] px-2 py-0.5 text-xs font-bold text-texto-fraco">
                  {itens.length}
                </span>
              </div>
              <div className="space-y-3">
                ...
                {itens.length === 0 ? (
                  <div className="rounded-lg border border-dashed border-linha p-4 text-center text-sm leading-6 text-texto-fraco">
